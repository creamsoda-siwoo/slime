...
const CLICK_UPGRADE_COST_MULTIPLIER = 1.2;
const DPS_UPGRADE_COST_BASE = 10;
const DPS_UPGRADE_COST_MULTIPLIER = 1.3;
...
const RARITIES = {
    ...
    UNCOMMON: { name: '고급', textClass: 'text-rarity-UNCOMMON', borderClass: 'border-rarity-UNCOMMON', statMultiplier: 1.5 },
    RARE: { name: '희귀', textClass: 'text-rarity-RARE', borderClass: 'border-rarity-RARE', statMultiplier: 2.5 },
    EPIC: { name: '영웅', textClass: 'text-rarity-EPIC', borderClass: 'border-rarity-EPIC', statMultiplier: 4 },
    LEGENDARY: { name: '전설', textClass: 'text-rarity-LEGENDARY', borderClass: 'border-rarity-LEGENDARY', statMultiplier: 6.5 }
};

// --- TYPES ---
// FIX: Define types for game state to resolve 'Property does not exist on type unknown' errors.
...
interface MonsterStatsType {
    name: string;
    // FIX: Qualify JSX.Element with React. to resolve "Cannot find namespace 'JSX'".
...
interface UpgradesType {
    ...
    critChanceUpgradeCost: number;
    // New
    goldGainUpgradeCost: number;
    xpGainUpgradeCost: number;
    psCooldownUpgradeCost: number;
    psDamageUpgradeCost: number;
}

interface AbilityStatsType {
    goldGainBonus: number;
    xpGainBonus: number;
    powerShotCooldownReduction: number;
...
interface GameState {
    ...
    monsterStats: MonsterStatsType;
    upgrades: UpgradesType;
    equipment: EquipmentType;
    gameLog: string[];
    abilityStats: AbilityStatsType;
...
const GreenSlimeIcon = () => (
    <svg viewBox="0 0 100 80" className="w-48 h-48 drop-shadow-lg">
...
const MonsterDisplay = ({ monster, onClick, damagePopups }) => {
    ...
    return (
        <div className="w-full md:w-1/2 bg-gray-800 p-6 rounded-lg shadow-lg flex flex-col items-center justify-center order-2 md:order-2 relative overflow-hidden">
            <div className={`transition-transform duration-100 ${shake ? 'animate-screen-shake' : ''}`}>
...
const UpgradePanel = ({ upgrades, playerCoins, handlers, abilityStats }) => (
...
const PowerShotButton = ({ onClick, cooldown, maxCooldown }) => {
    ...
    return (
        <button
            onClick={onClick}
            disabled={isDisabled}
            className="relative w-full md:w-auto bg-red-700 hover:bg-red-800 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold py-4 px-8 rounded-lg transition-colors shadow-lg overflow-hidden">
            {cooldown > 0 && (
...
const GameLog = ({ log }) => (
     <div className="w-full bg-gray-800 p-4 rounded-lg shadow-lg mt-4 order-4 h-32 overflow-y-auto">
...
const App = () => {
    ...
    const [playerStats, setPlayerStats] = useState<PlayerStatsType>(() => {
        ...
        try {
            const savedGame = JSON.parse(saved) as any;
            return { ...getDefaultState().playerStats, ...savedGame.playerStats };
        } catch (e) { return getDefaultState().playerStats; }
    });

    const [monsterStats, setMonsterStats] = useState<MonsterStatsType>(() => {
        const saved = localStorage.getItem(SAVED_GAME_KEY);
    ...
    const [abilityStats, setAbilityStats] = useState<AbilityStatsType>(() => {
        const saved = localStorage.getItem(SAVED_GAME_KEY);
    ...
    const [liberationCount, setLiberationCount] = useState<number>(() => {
        const saved = localStorage.getItem(SAVED_GAME_KEY);
    ...
    const [newItemDrop, setNewItemDrop] = useState<EquipmentItem | null>(null);
    const needsSave = useRef(false);

    const finalStats: PlayerStatsType = useMemo(() => {
        const combined = { ...playerStats };
        (Object.keys(equipment) as Array<keyof EquipmentType>).forEach(slot => {
    ...
    const generateRandomItem = useCallback((monsterLevel: number): EquipmentItem => {
        const rarityRoll = Math.random();
        let rarityKey: keyof typeof RARITIES = 'COMMON';
        ...
        const typeKeys = Object.keys(EQUIPMENT_TYPES) as (keyof typeof EQUIPMENT_TYPES)[];
    ...
    const handleMonsterDefeat = useCallback(() => {
        ...
        setPlayerStats(prev => {
            ...
            return { ...prev, level: newLevel, xp: newXp, maxXp: newMaxXp, coins: prev.coins + coinsGained };
        });
    ...
    const handleUpgradeDps = () => {
        if (playerStats.coins >= upgrades.dpsUpgradeCost) {
            ...
            setUpgrades(prev => ({ ...prev, dpsUpgradeCost: Math.floor(prev.dpsUpgradeCost * DPS_UPGRADE_COST_MULTIPLIER), dpsUpgradeLevel: prev.dpsUpgradeLevel + 1 }));
            needsSave.current = true;
        }
    };
    ...
    const handleUpgradePowerShotDamage = () => {
        if (playerStats.coins >= upgrades.psDamageUpgradeCost) {
            ...
            setUpgrades(prev => ({ ...prev, psDamageUpgradeCost: Math.floor(prev.psDamageUpgradeCost * PS_DAMAGE_UPGRADE_COST_MULTIPLIER) }));
            needsSave.current = true;
        }
    };
    ...
    const handleUpgradePowerShotCooldown = () => {
        if (playerStats.coins >= upgrades.psCooldownUpgradeCost) {
            ...
            setUpgrades(prev => ({ ...prev, psCooldownUpgradeCost: Math.floor(prev.psCooldownUpgradeCost * PS_COOLDOWN_UPGRADE_COST_MULTIPLIER) }));
            needsSave.current = true;
        }
    };
    ...
    const handleLiberation = () => {
        const cost = LIBERATION_COST_BASE * Math.pow(LIBERATION_COST_MULTIPLIER, liberationCount);
    ...
    const handleEquipNewItem = () => {
        if (!newItemDrop) return;
        const itemType = newItemDrop.type.toLowerCase() as keyof EquipmentType;
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-4 space-y-4">
            ...
            <footer className="w-full max-w-7xl flex flex-col md:flex-row items-center justify-center gap-4">
                 <PowerShotButton 
            ...
            <NewItemDropModal 
                newItem={newItemDrop}
                currentItem={newItemDrop ? equipment[newItemDrop.type.toLowerCase() as keyof EquipmentType] : null}
